...
<html>
<head>
<title>实时-GRIAFFE-每天都有新鲜事</title>
<meta name="viewport"
			...
			<c:forEach items="${livePosts}" var="p">
				...
				<tr
					...
					height="300">
						...
						<a href="javascript:praisePost('<%=pageContext.getServletContext().getContextPath() %>/praise/postlivepraisePost.action?praise_post=${p.post_id}&praise_user=${sessionScope.user.user_id}')">
								赞一下</a></td>
						<td style="font-size: large;text-align:center;vertical-align:middle;" width="240">
						<a href="javascript:praisePost('<%=pageContext.getServletContext().getContextPath() %>/favorite/postliveaddfavorite.action?favorite_post=${p.post_id}&favorite_user=${sessionScope.user.user_id}')">
								收藏</a></td>		
				</tr>
				<br></br>
				</c:forEach>
